...
import { fetchDroneByOrderId, completeDroneFlight } from '../../../js/drone-admin';
...
function OrderDetail({ show, handleCloseModal, order }) {
  ...
  useEffect(() => {
    ...
    console.log('near: ', nearOrder)
  }, [dronePos, orderPos]);

  const handleConfirmOrder = () => {
    updateStatus(order.id, 'completed'); // ví dụ cập nhật status
    ...
    <Modal show={show} onHide={handleCloseModal} centered backdrop keyboard={false} scrollable size="lg">
      ...
      <Modal.Body style={{ backgroundColor: 'white' }}>
        ...
        {order.status === 'delivering' && dronePos && orderPos && (
          <div style={{ height: '300px', marginTop: '1em', borderTop: '1px solid black' }}>
            <p style={{ fontWeight: 'lighter', fontSize: 'large', marginTop: '20px' }}>
              Drone: {droneId}
            </p>
        ...
        {nearOrder ? (
          <>
            {console.log("HIỆN BUTTON - nearOrder =", nearOrder)}
            <div style={{ textAlign: 'center', marginTop: '1em', zIndex: 99999 }}>
              <Button
          ...
          console.log("KHÔNG HIỆN BUTTON - nearOrder =", nearOrder)
        )}
